...
Use an older app version to check the new release. Confirm that automatic and manual checks from a `CN` public egress use the selected Gitee feed first and retry GitHub once, while other public egresses use GitHub first and retry Gitee once. Verify that Sparkle installs and relaunches the ZIP for the running architecture, leaving a single-architecture install on a single architecture. Test Release→Preview and Preview→Release switching as well as same-channel updates.

...
- **Copy assistant:** Runs locally on each `changeCount` when enabled. The quick trigger may use Input Monitoring for modifier-only keys and mouse side buttons; the optional quick-copy gesture also posts a synthetic ⌘C and therefore requires Accessibility. Recognition never connects to the network; opening links, searching, translating, and saving happen only after an explicit click or trigger press. An app blacklist and per-type switches can suppress it entirely.

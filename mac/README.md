...
| AI Agent | 管理项目与对话、远端模型及凭据、本地模型、消息连接、应用增强、自动化、CLI/Skills、会话归档与存储。 |
...
- 语音输入：支持按键切换或按住说话、自定义全局快捷键、系统语音识别、可选 AI 整理和本机录音历史。
...
- AI 状态：自动检测只读取判断任务状态所需的结构化事件和本地活动元数据；AI Agent 的远端凭据存入私有数据库，不写入普通设置或对话导出。
- 语音：仅在用户主动触发时访问麦克风和系统语音识别；录音、原始转写和 AI 整理文本由用户在本机管理。

...
// Adapted for Zisla from Mac Duo's Apache-2.0 `Homography`.
...
enum Homography {
    ...
    /// Maps a rectangle onto `corners` in the same convention, in points.
    static func matrix(width: CGFloat, height: CGFloat, to corners: [CGPoint]) -> simd_double3x3 {
        matrix(
            width: Double(width),

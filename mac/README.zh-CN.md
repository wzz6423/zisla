...
- 更新：当前选择的 Release 或 Preview 通道都先通过 Sparkle 检查带签名的 Gitee appcast；当 Gitee appcast 无法加载或更新包下载失败时，回退检查一次 GitHub appcast 并由 Sparkle 自动下载、验签、安装和重启；切换通道同时影响自动检查和手动检查。
...
- 网络：天气访问 Open-Meteo；中国大陆地点的官方预警访问中国天气网公开数据，其他地点优先使用 WeatherKit；当前选择的更新通道访问带签名的 Gitee appcast 和 Release ZIP，当 Gitee appcast 无法加载或更新包下载失败时回退一次 GitHub appcast 并由 Sparkle 自动下载；B 站备用下载访问其只读视频信息、播放地址和媒体 CDN；识别剪贴板链接本身不联网。

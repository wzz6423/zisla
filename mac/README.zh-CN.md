...
- 复制助手：每次复制后弹出小灵动岛提示，展示富内容预览和智能下一步动作 —— 打开链接、在 Finder 中显示、搜索（可选搜索引擎）、翻译、计算、从识别的日期新建日程，以及保存长文本和图片。支持识别 URL、文件路径、邮箱、电话号码（带实时色块的 CSS 颜色值）、算式、日期时间、中文文本、代码片段、带字数统计的纯文本、带缩略图的图片和文件引用。触发方式：操作按钮、连按修饰键（默认左 ⌃ Control）或鼠标侧键，均可自行配置；另提供可选的「按住左键 + 点右键」快速复制手势（默认关闭）、应用黑名单、识别类型开关和轻量提醒模式。

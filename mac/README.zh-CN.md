...
| 小工具 | 提供番茄钟（每段结束播放一次提示音并显示本机通知）、保持亮屏、防止空闲休眠、屏幕与键盘清洁、提词器、摄像头镜子和经确认后清空废纸篓。 |
...
- 快捷操作：每次复制后弹出小灵动岛提示，展示富内容预览和智能下一步动作 —— 打开链接、在 Finder 中显示、搜索（可选搜索引擎）、翻译、计算、从识别的日期新建日程，以及保存长文本和图片。浏览器下载完成或收到 AirDrop 文件后，也可用同一快捷键打开文件所在文件夹；这由快捷操作开关控制，与“浏览器下载进度”开关独立。支持识别 URL、文件路径、邮箱、电话号码（带实时色块的 CSS 颜色值）、算式、日期时间、非系统语言文本、代码片段、带字数统计的纯文本、带缩略图的图片和文件引用。触发方式：操作按钮、连按修饰键（默认左 ⌃ Control）或鼠标侧键，均可自行配置；另提供默认关闭的「按住左键 + 点右键」快速复制手势、应用黑名单、识别类型开关和轻量提醒模式。
- CLI 与 Skills 管理：在设置中检测、安装、更新和卸载常用 AI CLI，并管理本机 Skills。
- 桌面宠物：可选择内置或导入宠物，并配置显示在灵动岛左侧或右侧。
- 更新：当前选择的 Release 或 Preview 通道会在本次运行中解析一次公网出口国家码：`CN` 先通过 Sparkle 检查带签名的 Gitee appcast，再回退一次 GitHub；其他国家码先检查 GitHub，再回退一次 Gitee；无法获取国家码时保留 Gitee 优先。随后由 Sparkle 自动下载、验签、安装和重启；切换通道同时影响自动检查和手动检查。
...
| GitHub / Google / xAI | GitHub Copilot CLI 与 VS Code、Gemini CLI 与 Desktop、Grok CLI |
...
接入协议见 [CLI 接入设计](Docs/cli-reference.zh-CN.md)。

## 浏览器下载进度

zisla 使用 macOS 公共文件进度机制监听“下载”目录，并结合临时文件扩展名、下载来源扩展属性和正在运行的浏览器解析来源。它不会读取浏览器历史数据库，也不会为识别进度发起网络请求；下载进度的完成标记保留约 3 秒。快捷操作复用这套事件来源，仅在传输成功且文件到达最终位置后提供“打开文件夹”；提示时长跟随快捷操作设置，新的复制或传输提示会覆盖旧提示。
...
- 快捷操作：开启后在每次 `changeCount` 变化时本地处理，也处理已完成的浏览器下载和收到的 AirDrop 文件，不联网。单独修饰键和鼠标侧键触发可能需要输入监控权限；可选的快速复制手势还会模拟 ⌘C，需要辅助功能权限。打开链接、搜索、翻译和保存仅在用户明确点击或按下触发键后发生；应用黑名单与识别类型开关控制复制内容提示，快捷操作总开关同时控制传输完成提示。
...
- 网络：天气访问 Open-Meteo；中国大陆地点的官方预警访问中国天气网公开数据，其他地点优先使用 WeatherKit；更新检查会在每次进程运行中通过 `ipinfo.io/country` 查询一次公网出口国家码：`CN` 使用 Gitee 再回退 GitHub，其他国家码使用 GitHub 再回退 Gitee，无法获取国家码时保留 Gitee 优先；B 站备用下载访问其只读视频信息、播放地址和媒体 CDN；识别剪贴板链接本身不联网。
...
- [架构与性能设计](Docs/architecture.zh-CN.md)
- [CLI 接入设计](Docs/cli-reference.zh-CN.md)
- [签名与发布设计](Docs/releasing.zh-CN.md)
...
- 发布包未经公证，首次打开可能需要在系统设置中选择“仍要打开”。正式版使用同一张自签名证书，使 macOS 权限授予能在后续更新中保留；Sparkle 仍会独立使用应用内置 EdDSA 公钥验证每次更新，再解压、替换并重启应用。

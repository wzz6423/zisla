...
CodexActivityMonitor::CodexActivityMonitor(std::filesystem::path codex_root)
...
bool CodexActivityMonitor::start(HWND target, UINT changed_message) {
    if (thread_.joinable()) {
        return true;
    }
    if (!target || changed_message == 0 || codex_root_.empty()) {
        return false;
    }
...
void CodexActivityMonitor::stop() noexcept {
    if (stop_event_) {
...
bool CodexActivityMonitor::running() const noexcept {
    return running_.load(std::memory_order_acquire);
}

std::shared_ptr<const CodexActivityMonitor::ActivityList>
CodexActivityMonitor::snapshot() const noexcept {
    return snapshot_.load(std::memory_order_acquire);
}

std::optional<std::filesystem::path>
CodexActivityMonitor::defaultCodexRoot() noexcept {
    ...
    try {
        auto result = std::filesystem::path{profile} / L".codex";
...
void CodexActivityMonitor::run() noexcept {
    scanAndPublish();
...
bool CodexActivityMonitor::watchDirectory(
    HANDLE directory,
...
void CodexActivityMonitor::scanAndPublish() noexcept {
    try {
        const zisla::core::CodexSessionScanner scanner({
            .sessions_directory = codex_root_ / L"sessions",
            .session_index_path = codex_root_ / L"session_index.jsonl",
            .max_rollout_files = 4,
            .initial_tail_bytes = 256 * 1'024,
        });
...
bool CodexActivityMonitor::waitForStop(DWORD timeout_ms) const noexcept {
    if (!stop_event_) {
        return true;
    }

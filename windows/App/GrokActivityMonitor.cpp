...
GrokActivityMonitor::GrokActivityMonitor(std::filesystem::path sessions_directory)
    : sessions_directory_(std::move(sessions_directory)),
      snapshot_(std::make_shared<const ActivityList>()) {}

GrokActivityMonitor::~GrokActivityMonitor() {
    stop();
}

bool GrokActivityMonitor::start(HWND target, UINT changed_message) {
    if (thread_.joinable()) {
        return true;
    }
    if (!target || changed_message == 0 || sessions_directory_.empty()) {
        return false;
    }
...
void GrokActivityMonitor::stop() noexcept {
    if (stop_event_) {
...
bool GrokActivityMonitor::running() const noexcept {
    return running_.load(std::memory_order_acquire);
}

std::shared_ptr<const GrokActivityMonitor::ActivityList>
GrokActivityMonitor::snapshot() const noexcept {
    return snapshot_.load(std::memory_order_acquire);
}

std::optional<std::filesystem::path>
GrokActivityMonitor::defaultSessionsDirectory() noexcept {
    ...
    try {
        auto result = std::filesystem::path{profile} / L".grok" / L"sessions";
...
bool GrokActivityMonitor::watchDirectory(
    HANDLE directory,
...
void GrokActivityMonitor::scanAndPublish() noexcept {
    try {
        const zisla::core::GrokSessionScanner scanner({
            .sessions_directory = sessions_directory_,
...
bool GrokActivityMonitor::waitForStop(DWORD timeout_ms) const noexcept {
    if (!stop_event_) {
        return true;
    }

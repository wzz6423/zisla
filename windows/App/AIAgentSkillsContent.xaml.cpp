...
void AIAgentSkillsContent::updateView() {
    ...
    FileCopyModeButton().IsChecked(box_value(is_copy_mode).as<
        Windows::Foundation::IReference<bool>>());
    SymbolicLinkModeButton().IsChecked(box_value(!is_copy_mode).as<
        Windows::Foundation::IReference<bool>>());
...
void AIAgentSkillsContent::updateSkillRows() {
    ...
    for (const auto& skill : snapshot_->skills) {
        ...
        TextBlock name;
        name.Foreground(Microsoft::UI::Xaml::Application::Current().Resources().Lookup(
        ...
        TextBlock location;
        location.Foreground(Microsoft::UI::Xaml::Application::Current().Resources().Lookup(

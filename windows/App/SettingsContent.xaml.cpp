...
SettingsContent::SettingsContent() {
    InitializeComponent();
    std::wstring version_text{L"Zisla"};
    const auto application_version = AppHost::instance().applicationVersion();
    if (!application_version.empty()) {
        const auto version = to_hstring(application_version);
        version_text.append(L" ");
        version_text.append(version.c_str(), version.size());
    }
    AppVersionText().Text(hstring{version_text});
    TopEdgeToggle().IsOn(AppHost::instance().isTopEdgeEnabled());
    TaskbarWidgetToggle().IsOn(AppHost::instance().isTaskbarWidgetEnabled());
    updating_taskbar_widget_controls_ = true;
    const auto taskbar_position = AppHost::instance().taskbarWidgetPosition();
    TaskbarWidgetPositionPicker().SelectedIndex(
        taskbar_position == zisla::core::TaskbarWidgetPosition::leading ? 0
        : taskbar_position == zisla::core::TaskbarWidgetPosition::before_start ? 1
        : 2);
    updating_taskbar_widget_controls_ = false;
...
void SettingsContent::populateUpdateControls() {
    ...
    for (std::uint32_t index = 0; index < UpdateChannelPicker().Items().Size(); ++index) {
        ...
        if (item && to_string(unbox_value_or<hstring>(item.Tag(), L"")) == token) {
...
SettingsContent::selectedUpdateChannel() {
...
void SettingsContent::Navigation_SelectionChanged(
    ...
    const auto tag = item
        ...
        : hstring{L"general"};
    using Microsoft::UI::Xaml::Visibility;
    const auto page_visibility = [&tag](hstring const& page_tag) {
        return tag == page_tag ? Visibility::Visible : Visibility::Collapsed;
    };
    GeneralPage().Visibility(page_visibility(L"general"));
    ClipboardPage().Visibility(page_visibility(L"clipboard"));
    InfoPage().Visibility(page_visibility(L"info"));
    InteractionPage().Visibility(page_visibility(L"interaction"));
    MailPage().Visibility(page_visibility(L"mail"));
    UpdatesPage().Visibility(page_visibility(L"updates"));
    AboutPage().Visibility(page_visibility(L"about"));
}
...
void SettingsContent::TaskbarWidgetPositionPicker_SelectionChanged(
    Windows::Foundation::IInspectable const&,
    Microsoft::UI::Xaml::Controls::SelectionChangedEventArgs const&) {
    if (updating_taskbar_widget_controls_) {
        return;
    }
    const auto item = TaskbarWidgetPositionPicker().SelectedItem().try_as<
        Microsoft::UI::Xaml::Controls::ComboBoxItem>();
    if (!item) {
        return;
    }
    const auto token = unbox_value_or<hstring>(item.Tag(), L"trailing");
    AppHost::instance().setTaskbarWidgetPosition(
        token == L"leading" ? zisla::core::TaskbarWidgetPosition::leading
        : token == L"before-start" ? zisla::core::TaskbarWidgetPosition::before_start
        : zisla::core::TaskbarWidgetPosition::trailing);
}

...
KimiActivityMonitor::KimiActivityMonitor(std::filesystem::path home_directory)
    : home_directory_(std::move(home_directory)),
      snapshot_(std::make_shared<const ActivityList>()) {}

KimiActivityMonitor::~KimiActivityMonitor() {
    stop();
}

bool KimiActivityMonitor::start(HWND target, UINT changed_message) {
    if (thread_.joinable()) {
        return true;
    }
    if (!target || changed_message == 0 || home_directory_.empty()) {
        return false;
    }
...
void KimiActivityMonitor::stop() noexcept {
    if (stop_event_) {
...
bool KimiActivityMonitor::running() const noexcept {
    return running_.load(std::memory_order_acquire);
}

std::shared_ptr<const KimiActivityMonitor::ActivityList>
KimiActivityMonitor::snapshot() const noexcept {
    return snapshot_.load(std::memory_order_acquire);
}

std::optional<std::filesystem::path>
KimiActivityMonitor::defaultHomeDirectory() noexcept {
    if (const auto configured = configured_home_directory()) {
        return configured;
    }
    ...
    try {
        auto result = std::filesystem::path{profile} / L".kimi-code";
...
void KimiActivityMonitor::run() noexcept {
    scanAndPublish();
    auto last_refresh_tick = GetTickCount64();
    const auto sessions_directory = home_directory_ / L"sessions";

    while (!waitForStop(0)) {
        const auto raw_directory = CreateFileW(
            sessions_directory.c_str(),
            FILE_LIST_DIRECTORY,
...
bool KimiActivityMonitor::watchDirectory(
    HANDLE directory,
...
void KimiActivityMonitor::scanAndPublish() noexcept {
    try {
        const zisla::core::KimiSessionScanner scanner({
            .home_directory = home_directory_,
            .max_session_files = 12,
            .initial_tail_bytes = 1'024 * 1'024,
            .maximum_state_bytes = 1'024 * 1'024,
        });
...
bool KimiActivityMonitor::waitForStop(DWORD timeout_ms) const noexcept {
    if (!stop_event_) {
        return true;
    }

...
void OverlayContent::updateQuickNotesView() {
    ...
    NotesEditor().IsEnabled(has_selection);
    NotesEditor().Visibility(has_selection ? Visibility::Visible : Visibility::Collapsed);
...
void OverlayContent::updateSystemMonitorView() {
    ...
    SystemCPUUsage().Text(metrics.cpu ? system_percent(cpu_usage) : hstring{L"--%"});
    SystemCPUProgress().IsIndeterminate(!metrics.cpu.has_value());
    SystemCPUProgress().Value(cpu_usage);
    if (metrics.cpu) {
        std::wstring detail = L"用户 ";
        append(detail, system_percent(metrics.cpu->user_fraction));
        detail.append(L" · 系统 ");
        append(detail, system_percent(metrics.cpu->system_fraction));
        detail.append(L" · 闲置 ");
        append(detail, system_percent(metrics.cpu->idle_fraction));
        SystemCPUDetail().Text(hstring{detail});
    } else {
        SystemCPUDetail().Text(L"正在建立采样基线");
    }
    update_fraction_history(
        SystemCPUHistoryLine(),
        SystemCPUHistoryCanvas(),
        snapshot->history.cpu_usage());

    SystemGPUName().Text(metrics.hardware.gpu_name.empty()
        ? hstring{L"图形处理器"}
        : from_utf8(metrics.hardware.gpu_name));
    const auto gpu_usage = metrics.gpu ? metrics.gpu->usage_fraction : 0.0;
    SystemGPUUsage().Text(metrics.gpu ? system_percent(gpu_usage) : hstring{L"--%"});
    SystemGPUProgress().IsIndeterminate(false);
    SystemGPUProgress().Value(gpu_usage);
    SystemGPUDetail().Text(metrics.gpu
        ? hstring{L"GPU Engine 性能计数器"}
        : hstring{L"GPU 性能计数器不可用"});
    update_fraction_history(
    ...
    const bool trimming_memory = desktop_busy
        && desktop_tools->active_action
            == zisla::core::DesktopToolAction::trim_own_working_set;
    SystemMemoryTrimButton().IsEnabled(!desktop_busy);
    SystemMemoryTrimProgress().IsActive(trimming_memory);
    SystemMemoryTrimProgress().Visibility(
        trimming_memory ? Visibility::Visible : Visibility::Collapsed);
    hstring trim_status;
    if (desktop_tools
    ...
    SystemDiskUsage().Text(metrics.disk.available
        ...
        : hstring{L"--%"});
    SystemDiskProgress().Value(
        metrics.disk.available ? metrics.disk.usage_fraction : 0.0);
    std::wstring disk_detail;
    if (metrics.disk.available) {
        if (!metrics.disk.volume_name.empty()) {
            append(disk_detail, from_utf8(metrics.disk.volume_name));
        ...
        append(disk_detail, metrics.disk.read_bytes_per_second
            ? from_utf8(zisla::core::SystemMonitorFormat::rate(
                *metrics.disk.read_bytes_per_second))
            : hstring{L"--"});
        disk_detail.append(L" · 写 ");
        append(disk_detail, metrics.disk.write_bytes_per_second
            ? from_utf8(zisla::core::SystemMonitorFormat::rate(
                *metrics.disk.write_bytes_per_second))
            : hstring{L"--"});
    } else {
        disk_detail = L"磁盘数据不可用";
    }
    ...
    SystemNetworkUpload().Text(metrics.network.available
        ...
        : hstring{L"上传 --"});
    SystemPrivateIP().Text(hstring{L"内网 IP "
        + (metrics.network.private_ip_address.empty()
            ? std::wstring{L"--"}
            : std::wstring{from_utf8(metrics.network.private_ip_address).c_str()})});
    SystemPublicIP().Text(hstring{L"公网 IP "
        + (metrics.network.public_ip_address.empty()
            ? std::wstring{L"--"}
            : std::wstring{from_utf8(metrics.network.public_ip_address).c_str()})});

    std::wstring battery = L"电池 ";
    if (!metrics.battery) {
        battery.append(L"状态不可用");
    } else if (!metrics.battery->present) {
        battery.append(L"未检测到电池");
    } else {
    ...
    SystemUptime().Text(hstring{L"运行时间 "
        + std::wstring{from_utf8(zisla::core::SystemMonitorFormat::uptime(
            metrics.uptime_seconds)).c_str()}});
    SystemSensorStatus().Text(L"温度与风扇：Windows 暂无统一可靠的公开桌面接口");

    std::wstring accessible_name = L"系统监控，CPU ";
...
void OverlayContent::updateDiskCleanupView() {
    ...
    if (empty) {
        DiskCleanupEmptyText().Text(snapshot && snapshot->revision > 0
            ? hstring{L"没有可清理项目"}
            : hstring{L"尚未扫描"});
    }

    if (!snapshot || snapshot->revision == 0) {
        DiskCleanupSummaryText().Text(L"尚未扫描");
    } else if (snapshot->scanning) {
        DiskCleanupSummaryText().Text(L"正在扫描");
    } else if (snapshot->cleaning) {
        DiskCleanupSummaryText().Text(L"正在移入回收站");
    } else {
        std::wstring summary = std::to_wstring(snapshot->candidates.size());
        summary.append(L" 项");
        if (total_bytes > 0) {
            summary.append(L" · ");
        ...
        DiskCleanupSummaryText().Text(hstring{summary});
    }
...
void OverlayContent::SystemMemoryTrimButton_Click(
    Windows::Foundation::IInspectable const&,
    Microsoft::UI::Xaml::RoutedEventArgs const&) {
    AppHost::instance().trimOwnWorkingSet();
}
...
void OverlayContent::updateMailView() {
    ...
    case MailServicePhase::ready: {
        std::wstring ready = snapshot->message.empty()
            ? L"收件箱已更新"
            : std::wstring{from_utf8(snapshot->message).c_str()};
        ready.append(L" · ");
        ready.append(std::to_wstring(snapshot->messages.size()));
        ready.append(L" 封邮件");
        status = hstring{ready};
        break;
    }
...
winrt::fire_and_forget OverlayContent::AgendaNewEventButton_Click(
    ...
    const auto lifetime = get_strong();
    (void)lifetime;
    co_await showCalendarEditor(false);
}
...
winrt::fire_and_forget OverlayContent::AgendaNewReminderButton_Click(
    ...
    const auto lifetime = get_strong();
    (void)lifetime;
    co_await showCalendarEditor(true);
}
...
std::vector<zisla::core::MailRecipient> OverlayContent::mailRecipients() const {

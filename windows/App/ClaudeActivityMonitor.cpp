...
ClaudeActivityMonitor::ClaudeActivityMonitor(std::filesystem::path projects_root)
    : projects_root_(std::move(projects_root)),
      snapshot_(std::make_shared<const ActivityList>()) {}

ClaudeActivityMonitor::~ClaudeActivityMonitor() {
    stop();
}

bool ClaudeActivityMonitor::start(HWND target, UINT changed_message) {
    if (thread_.joinable()) {
        return true;
    }
    if (!target || changed_message == 0 || projects_root_.empty()) {
        return false;
    }
...
void ClaudeActivityMonitor::stop() noexcept {
    if (stop_event_) {
...
bool ClaudeActivityMonitor::running() const noexcept {
    return running_.load(std::memory_order_acquire);
}

std::shared_ptr<const ClaudeActivityMonitor::ActivityList>
ClaudeActivityMonitor::snapshot() const noexcept {
    return snapshot_.load(std::memory_order_acquire);
}

std::optional<std::filesystem::path>
ClaudeActivityMonitor::defaultProjectsRoot() noexcept {
    ...
    try {
        auto result = std::filesystem::path{profile} / L".claude" / L"projects";
...
bool ClaudeActivityMonitor::watchDirectory(
    HANDLE directory,
...
void ClaudeActivityMonitor::scanAndPublish() noexcept {
    try {
        const zisla::core::ClaudeSessionScanner scanner({
            .projects_directory = projects_root_,
...
bool ClaudeActivityMonitor::waitForStop(DWORD timeout_ms) const noexcept {
    if (!stop_event_) {
        return true;
    }

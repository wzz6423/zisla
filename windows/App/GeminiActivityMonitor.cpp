...
GeminiActivityMonitor::GeminiActivityMonitor(std::filesystem::path sessions_root)
    : sessions_root_(std::move(sessions_root)),
      snapshot_(std::make_shared<const ActivityList>()) {}

GeminiActivityMonitor::~GeminiActivityMonitor() {
    stop();
}

bool GeminiActivityMonitor::start(HWND target, UINT changed_message) {
    if (thread_.joinable()) {
        return true;
    }
    if (!target || changed_message == 0 || sessions_root_.empty()) {
        return false;
    }
...
void GeminiActivityMonitor::stop() noexcept {
    if (stop_event_) {
...
bool GeminiActivityMonitor::running() const noexcept {
    return running_.load(std::memory_order_acquire);
}

std::shared_ptr<const GeminiActivityMonitor::ActivityList>
GeminiActivityMonitor::snapshot() const noexcept {
    return snapshot_.load(std::memory_order_acquire);
}

std::optional<std::filesystem::path>
GeminiActivityMonitor::defaultSessionsRoot() noexcept {
    ...
    try {
        auto result = std::filesystem::path{profile} / L".gemini" / L"tmp";
...
bool GeminiActivityMonitor::watchDirectory(
    HANDLE directory,
...
void GeminiActivityMonitor::scanAndPublish() noexcept {
    try {
        const zisla::core::GeminiSessionScanner scanner({
            .sessions_directory = sessions_root_,
            .max_session_files = 12,
            .initial_tail_bytes = 1'024 * 1'024,
            .maximum_legacy_json_bytes = 1'024 * 1'024,
        });
...
bool GeminiActivityMonitor::waitForStop(DWORD timeout_ms) const noexcept {
    if (!stop_event_) {
        return true;
    }

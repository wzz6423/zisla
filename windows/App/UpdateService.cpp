...
SourceResult fetch_release(zisla::core::ReleaseSource source,
    ...
    try {
        ...
        const auto releases = zisla::core::ReleaseResponseParser::parse_list(
            response.body,
            source);
        return {
            .release = zisla::core::UpdateSelector::select_latest(releases, channel),
            .succeeded = true,
        };
...
void UpdateService::execute(Command command) {
    if (!zisla::core::SemanticVersion::parse(current_version_)) {
        throw std::runtime_error("无法读取当前应用版本");
    }
    publish({
    ...
    if (!gitee.succeeded && (!github || !github->succeeded)) {

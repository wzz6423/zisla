...
struct DockNoticeState {
    std::optional<zisla::core::IslandNotice> notice;
    std::size_t count{0};
};

DockNoticeState dockNoticeFromQueue(const zisla::core::SideNoticeQueue& queue) {
    constexpr std::array priorities{
        zisla::core::CompactStatusPriority::ai_activity,
        zisla::core::CompactStatusPriority::transient,
        zisla::core::CompactStatusPriority::video_download,
        zisla::core::CompactStatusPriority::browser_download,
        zisla::core::CompactStatusPriority::focus_countdown,
        zisla::core::CompactStatusPriority::toolbox_reminder,
        zisla::core::CompactStatusPriority::focus_mode,
        zisla::core::CompactStatusPriority::media,
    };
    for (const auto side : {zisla::core::NoticeSide::right, zisla::core::NoticeSide::left}) {
        const auto state = queue.view_state(side, priorities);
        if (state.compact_notice
            && !state.compact_notice->id.starts_with("media-active-")) {
            return {state.compact_notice, state.compact_count};
        }
        if (!state.ordinary_notices.empty()) {
            return {state.ordinary_notices.back(), state.ordinary_notices.size()};
        }
    }
    return {};
}

DockNoticeState activeDockNotice(
    const zisla::core::SideNoticeQueue& tray_queue,
    const zisla::core::SideNoticeQueue& side_queue) {
    auto result = dockNoticeFromQueue(tray_queue);
    return result.notice ? result : dockNoticeFromQueue(side_queue);
}

std::string packaged_application_version() noexcept {
    try {
...
zisla::core::TaskbarWidgetPosition taskbar_widget_position_from_token(
    hstring const& value) noexcept {
    if (value == L"leading") {
        return zisla::core::TaskbarWidgetPosition::leading;
    }
    if (value == L"before-start" || value == L"center") {
        return zisla::core::TaskbarWidgetPosition::before_start;
    }
    return zisla::core::TaskbarWidgetPosition::trailing;
}

hstring taskbar_widget_position_token(
    zisla::core::TaskbarWidgetPosition value) noexcept {
    switch (value) {
    case zisla::core::TaskbarWidgetPosition::leading:
        return L"leading";
    case zisla::core::TaskbarWidgetPosition::before_start:
        return L"before-start";
    case zisla::core::TaskbarWidgetPosition::trailing:
        return L"trailing";
    }
    return L"trailing";
}
...
void AppHost::loadSettings() {
    try {
        ...
        if (values.HasKey(L"TaskbarWidgetPosition")) {
            instance().taskbar_widget_position_ = taskbar_widget_position_from_token(
                winrt::unbox_value_or<hstring>(
                    values.Lookup(L"TaskbarWidgetPosition"),
                    L"trailing"));
        }
...
void AppHost::saveSettings() {
    try {
        ...
        const bool taskbar_widget_position_replaced = values.Insert(
            L"TaskbarWidgetPosition",
            winrt::box_value(taskbar_widget_position_token(
                instance().taskbar_widget_position_)));
        (void)taskbar_widget_position_replaced;
...
void AppHost::start() {
    ...
    refreshVoiceInput();
    refreshVoiceHotkey();
    taskbar_button_window_ = std::make_unique<TaskbarButtonWindow>(
        message_window_,
        activate_message,
        taskbar_exit_message);
    taskbar_button_window_->setEnabled(taskbar_widget_enabled_);
    ...
    update_service_ = std::make_unique<UpdateService>(applicationVersion());
    ...
    // The normal Windows taskbar button is the primary entry. Only keep a
    // notification-area icon when that supported integration is disabled.
    const bool taskbar_button_available = taskbar_button_window_
        && taskbar_button_window_->enabled();
    const bool tray_available = !taskbar_button_available
        && tray_icon_.add(message_window_, tray_message);
    ...
    if (!tray_available && !top_edge_available && !taskbar_button_available) {
        showSettings();
    }
...
void AppHost::shutdown() noexcept {
    ...
    tray_notice_window_.reset();
    system_monitor_service_.reset();
    desktop_tools_service_.reset();
    disk_cleanup_service_.reset();
    download_service_.reset();
    browser_download_service_.reset();
    taskbar_button_window_.reset();
    pet_entries_.clear();
    pet_activity_.reset();
...
LRESULT CALLBACK AppHost::windowProcedure(
    ...
    if (host) {
        if (host->taskbar_created_message_ != 0
            ...
            if (host->taskbar_button_window_) {
                host->taskbar_button_window_->refreshAfterExplorerRestart();
            }
            if (!host->taskbar_widget_enabled_) {
...
LRESULT AppHost::handleMessage(
    ...
    case WM_TIMER:
        ...
        if (wparam == side_notice_timer_id) {
            ...
            (void)tray_notice_queue_.remove_expired(monotonic_milliseconds());
...
bool AppHost::showOverlay(zisla::core::OverlayAnchor anchor) {
    ...
    try {
        ...
        const auto preferred_size = surface == zisla::core::OverlaySurfaceKind::interactive
            ? std::optional{overlay_window_->preferredInteractiveCardSize()}
            : std::optional<zisla::core::DipSize>{};
        const auto bounds = overlayBounds(anchor, surface, preferred_size);
        if (!bounds) {
            return false;
        }
...
void AppHost::refreshNowPlaying() noexcept {
    try {
        const auto snapshot = media_session_monitor_
            ...
            : nullptr;
        now_playing_snapshot_ = snapshot && snapshot->valid() ? snapshot : nullptr;
        if (overlay_window_) {
            overlay_window_->setNowPlaying(now_playing_snapshot_);
        }
        refreshTaskbarWidget();
...
void AppHost::refreshTaskbarWidget() noexcept {
    if (!taskbar_button_window_) {
        return;
    }

    // Explorer owns its visual tree. A normal application button is the
    // supported taskbar surface and remains stable across Explorer updates.
    taskbar_button_window_->setEnabled(taskbar_widget_enabled_);
    if (taskbar_widget_enabled_) {
        const auto notice = activeDockNotice(
            tray_notice_queue_,
            side_notice_queue_);
        taskbar_button_window_->setDockNotice(notice.notice, notice.count);
        taskbar_button_window_->setNowPlaying(now_playing_snapshot_);
        std::optional<zisla::core::PetLibraryEntry> pet;
        if (settings_.pet_enabled && !pet_entries_.empty()) {
        ...
        taskbar_button_window_->setPet(std::move(pet));
    }
}
...
void AppHost::refreshBackdrops() noexcept {
    ...
    if (tray_notice_window_) {
        tray_notice_window_->refreshBackdrop();
    }
}

std::optional<zisla::core::PixelRect> AppHost::overlayBounds(
    zisla::core::OverlayAnchor anchor,
    zisla::core::OverlaySurfaceKind surface,
    std::optional<zisla::core::DipSize> preferred_size) {
    ...
    if (anchor == zisla::core::OverlayAnchor::tray) {
        ...
        const auto taskbar = TaskbarPlacement::current();
        ...
        const auto screen = DisplayTopology::screenForPoint(cursor);
        bounds = placement_engine_.topEdgeCard(screen, surface, preferred_size);
    }
...
void AppHost::requestOverlayRelayout() noexcept {
    if (!overlay_window_
        || !overlay_window_->visible()
        || presentation_engine_.state().visibility
            != zisla::core::OverlayVisibility::interactive) {
        return;
    }

    try {
        const auto bounds = overlayBounds(
            presentation_engine_.state().anchor,
            zisla::core::OverlaySurfaceKind::interactive,
            overlay_window_->preferredInteractiveCardSize());
        if (bounds) {
            overlay_window_->updateBounds(*bounds);
        }
    } catch (...) {
    }
}
...
void AppHost::refreshBrowserDownloadNotice(
    ...
    if (!snapshot || !notices_enabled) {
        (void)tray_notice_queue_.remove(active_notice_id);
        browser_completed_notice_ids_.clear();
        return;
    }
    ...
    if (snapshot->summary.total_active_count == 0) {
        ...
        if (!tray_notice_queue_.update_if_present(notice)) {
            tray_notice_queue_.enqueue(
                notice,
                monotonic_milliseconds(),
                std::nullopt);
        }
...
void AppHost::refreshAIActivityNotices(
    ...
    if (!settings_.side_notices_enabled || settings_.notifications_muted) {
        for (const auto& id : active_ai_notice_ids_) {
            (void)tray_notice_queue_.remove(id);
        }
        active_ai_notice_ids_.clear();
        shown_ai_notice_ids_.clear();
        ai_task_statuses_.clear();
    ...
    bool side_notice_enqueued = false;
...
void AppHost::scheduleSideNoticeExpiration() noexcept {
    ...
    auto next = side_notice_queue_.next_expiration_ms();
    if (const auto tray_next = tray_notice_queue_.next_expiration_ms();
        tray_next && (!next || *tray_next < *next)) {
        next = tray_next;
    }
    if (!next) {
        return;
...
void AppHost::setTaskbarWidgetEnabled(bool enabled) {
    ...
    if (message_window_) {
        if (enabled) {
            ...
            (void)tray_icon_.add(message_window_, tray_message);
        }
    }
    if (taskbar_button_window_) {
        taskbar_button_window_->setEnabled(enabled);
    }
...
void AppHost::setTaskbarWidgetPosition(
    zisla::core::TaskbarWidgetPosition position) {
    if (taskbar_widget_position_ == position) {
        return;
    }
    taskbar_widget_position_ = position;
    refreshTaskbarWidget();
...
void AppHost::setBrowserDownloadStatusEnabled(bool enabled) {
    ...
    if (!enabled) {
        browser_download_service_.reset();
        (void)tray_notice_queue_.remove("browser-download-active-right");
...
void AppHost::releaseSystemMemory() {
    if (desktop_tools_service_) {
        desktop_tools_service_->releaseSystemMemory();
    }
}
...
void AppHost::setSideNoticeHovered(std::string_view id, bool hovered) {
    const auto now = monotonic_milliseconds();
    if (side_notice_queue_.set_hovered(id, hovered, now)
        || tray_notice_queue_.set_hovered(id, hovered, now)) {
        scheduleSideNoticeExpiration();
    }
}

void AppHost::dismissSideNotice(std::string_view id) {
    if (side_notice_queue_.remove(id) || tray_notice_queue_.remove(id)) {
        updateSideNoticeWindows();
    }
}
...
zisla::core::TaskbarWidgetPosition AppHost::taskbarWidgetPosition() const noexcept {
    return taskbar_widget_position_;
}
...
std::string AppHost::applicationVersion() const {

...
HarnessActivityMonitor::HarnessActivityMonitor(std::filesystem::path data_directory)
    : data_directory_(std::move(data_directory)),
      snapshot_(std::make_shared<const ActivityList>()) {}

HarnessActivityMonitor::~HarnessActivityMonitor() {
    stop();
}

bool HarnessActivityMonitor::start(HWND target, UINT changed_message) {
    if (thread_.joinable()) {
        return true;
    }
    if (!target || changed_message == 0 || data_directory_.empty()) {
        return false;
    }
...
void HarnessActivityMonitor::stop() noexcept {
    if (stop_event_) {
...
bool HarnessActivityMonitor::running() const noexcept {
    return running_.load(std::memory_order_acquire);
}

std::shared_ptr<const HarnessActivityMonitor::ActivityList>
HarnessActivityMonitor::snapshot() const noexcept {
    return snapshot_.load(std::memory_order_acquire);
}

std::optional<std::filesystem::path>
HarnessActivityMonitor::defaultDataDirectory() noexcept {
    ...
    try {
        auto result = std::filesystem::path{profile} / L".harnext";
...
void HarnessActivityMonitor::run() noexcept {
    scanAndPublish();
...
bool HarnessActivityMonitor::watchDirectory(
    HANDLE directory,
...
void HarnessActivityMonitor::scanAndPublish() noexcept {
    try {
        const zisla::core::HarnessSessionScanner scanner({
...
bool HarnessActivityMonitor::waitForStop(DWORD timeout_ms) const noexcept {
    if (!stop_event_) {
        return true;
    }

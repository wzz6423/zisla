...
void trim_own_working_set() {
    if (!K32EmptyWorkingSet(GetCurrentProcess())) {
        throw std::runtime_error(
            "无法释放 Zisla 内存（错误 "
...
void DesktopToolsService::trimOwnWorkingSet() {
    enqueue(zisla::core::DesktopToolAction::trim_own_working_set);
}
...
void DesktopToolsService::execute(zisla::core::DesktopToolAction action) {
    ...
    case zisla::core::DesktopToolAction::open_store_updates:
        ...
        return;
    case zisla::core::DesktopToolAction::trim_own_working_set:
        trim_own_working_set();
        state_.complete(
            query_recycle_bin_or(previous),
            "已请求 Windows 释放 Zisla 未使用的内存页");
        return;

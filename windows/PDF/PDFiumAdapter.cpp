...
struct FileWriterContext {
    FPDF_FILEWRITE file_writer{1, nullptr};
...
RGBAImage decode_png(std::span<const std::uint8_t> bytes) {
    ...
    for (std::size_t y = 0; y < height; ++y) {
        ...
        for (std::size_t x = 0; x < row_bytes; ++x) {
            const auto left = x >= channels ? target[x - channels] : std::uint8_t{0};
            const auto up = previous ? previous[x] : std::uint8_t{0};
            const auto upper_left = previous && x >= channels ? previous[x - channels] : std::uint8_t{0};

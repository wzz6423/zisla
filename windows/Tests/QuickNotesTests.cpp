...
void markdownBlocksCoverTheSupportedSyntax() {
    const auto blocks = quick_note_markdown_blocks(
        "# 标题\n"
        ...
        "int value = 1;\n"
        "```\n");
    expect(blocks.size() == 7, "supported Markdown syntax should create seven blocks");
    expect(blocks[0].kind == QuickNoteMarkdownBlockKind::heading
            && blocks[0].heading_level == 1
            && blocks[0].inlines[0].text == "标题",
        "ATX headings should preserve their level and text");
    expect(blocks[1].kind == QuickNoteMarkdownBlockKind::paragraph
            && blocks[1].inlines.size() == 11
            && blocks[1].inlines[1].kind == QuickNoteMarkdownInlineKind::strong
            && blocks[1].inlines[3].kind == QuickNoteMarkdownInlineKind::emphasis
            && blocks[1].inlines[5].kind == QuickNoteMarkdownInlineKind::strikethrough
            && blocks[1].inlines[7].kind == QuickNoteMarkdownInlineKind::code,
        "basic inline formatting should be tokenized in reading order");
    expect(blocks[1].inlines[9].kind == QuickNoteMarkdownInlineKind::link
            && blocks[1].inlines[10].kind == QuickNoteMarkdownInlineKind::text,
        "links and trailing text should remain visible");
    expect(blocks[2].kind == QuickNoteMarkdownBlockKind::quote,
        "block quotes should be recognized");
    expect(blocks[3].kind == QuickNoteMarkdownBlockKind::unordered_list_item,
        "unordered list items should be recognized");
    expect(blocks[4].kind == QuickNoteMarkdownBlockKind::ordered_list_item
            && blocks[4].list_ordinal == 3,
        "ordered list items should retain their ordinal");
    expect(blocks[5].kind == QuickNoteMarkdownBlockKind::thematic_break,
        "thematic breaks should be recognized");
    expect(blocks[6].kind == QuickNoteMarkdownBlockKind::code_block
            && blocks[6].code == "int value = 1;",
        "fenced code blocks should preserve their content without the fence");

    const auto link_blocks = quick_note_markdown_blocks("[链接](https://example.com)");
    expect(link_blocks.size() == 1 && link_blocks[0].inlines.size() == 1
            && link_blocks[0].inlines[0].kind == QuickNoteMarkdownInlineKind::link
            && link_blocks[0].inlines[0].link == "https://example.com",
        "links should retain both their label and destination");
}
...
int main() {
    const std::pair<std::string_view, std::function<void()>> tests[] = {
        ...
        {"Markdown blocks cover supported syntax", markdownBlocksCoverTheSupportedSyntax},

...
void completionRecordsTheActionThatFinished() {
    DesktopToolsState state;
    expect(state.begin(DesktopToolAction::trim_own_working_set),
        "trimming the application working set should start");
    state.complete({.available = true}, "trimmed");

    expect(state.snapshot().last_action
            == DesktopToolAction::trim_own_working_set,

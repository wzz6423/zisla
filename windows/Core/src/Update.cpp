...
JsonDocument parse_release_document(std::string_view response) {
    if (response.size() > ReleaseResponseParser::maximum_response_bytes) {
...
std::string release_page_url(ReleaseSource source, std::string_view tag) {
    const auto prefix = source == ReleaseSource::gitee
        ? std::string_view{"https://gitee.com/wzz6423/zisla/releases/tag/"}
        : std::string_view{"https://github.com/wzz6423/zisla/releases/tag/"};
    return std::string(prefix).append(tag);
}

ReleaseInfo parse_release_info(
    yyjson_val* root,
    std::optional<ReleaseSource> fallback_source) {
    ...
    auto html_url = string_member(root, "html_url");
    if (tag && (!html_url || html_url->empty()) && fallback_source) {
        html_url = release_page_url(*fallback_source, *tag);
    }
...
std::string_view release_endpoint(
    ...
    switch (source) {
    case ReleaseSource::gitee:
        return "https://gitee.com/api/v5/repos/wzz6423/zisla/releases?per_page=100";
    case ReleaseSource::github:
        return "https://api.github.com/repos/wzz6423/zisla/releases?per_page=100";
    }
    return "https://api.github.com/repos/wzz6423/zisla/releases?per_page=100";
}
...
ReleaseInfo ReleaseResponseParser::parse(std::string_view response) {
    const auto document = parse_release_document(response);
    auto* root = yyjson_doc_get_root(document.get());
    return parse_release_info(root, std::nullopt);
}

std::vector<ReleaseInfo> ReleaseResponseParser::parse_list(
    ...
    yyjson_arr_foreach(root, index, maximum, item) {
        releases.push_back(parse_release_info(item, source));
    }
    return releases;
}

std::optional<ReleaseInfo> UpdateSelector::select_latest(
    ...
    for (const auto& release : releases) {
        ...
        if (!selected_version || *selected_version < *version) {
            selected = release;

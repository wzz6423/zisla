...
void apply_metadata(yyjson_val* root, SessionState& state) {
    ...
    } else if (const auto legacy_session_id = string_member(root, "session_id")) {
        state.session_id = *legacy_session_id;
    }
    if (!state.started_at_unix_ms) {
        if (const auto started_at = timestamp_member(root, "startTime")) {
            state.started_at_unix_ms = *started_at;
        } else if (const auto legacy_started_at = timestamp_member(root, "start_time")) {
            state.started_at_unix_ms = *legacy_started_at;
        }
    }
}

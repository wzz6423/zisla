...
void apply_user_record(
    ...
    for_each_content_item(message, [&](yyjson_val* item) {
        ...
        } else if (const auto legacy_id = copied_trimmed_string(yyjson_obj_get(item, "toolUseId"))) {
            state.pending_ask_tool_use_ids.erase(*legacy_id);
        }
...
std::optional<AIProgressTask> task_from_transcript(
    ...
    for_each_complete_line(transcript.jsonl, [&](std::string_view line) {
        ...
        } else if (const auto legacy_session_id = copied_trimmed_string(
                       yyjson_obj_get(root, "session_id"))) {
            fallback_session_id = *legacy_session_id;
        }

...
export const crossModuleFeatures: CrossModuleFeature[] = [
  ...
  },
  {
    ...
    title: '键盘音效与全局监听',
    description:
      '为全局键盘按下与释放播放可选音色，支持 20 种内置机械键盘音色、音量、自然音高变化、回弹音开关和试听；只有开启后才监听全局键盘事件。',
    detail: '20 种内置音色 · 按下 / 释放音 · 输入监控权限',
  },
  {
    id: 'typingStats',
    ...
    title: '本地输入统计',
    description:
      '可选开启的本地聚合统计，按字符数、物理按键、时间和前台应用记录输入趋势；提供今日概览、历史区间、应用时间线与逐键热力图，不保存输入内容。',
    detail: '默认关闭 · 趋势与历史 · 应用时间线与热力图',
  },
];
  ...
  },
  {
    ...
    answer: `
      ...
      <ul>
        ...
        <li><strong>输入监控：</strong>键盘音效、可选的本地输入统计、键盘清洁，以及使用单独修饰键或鼠标侧键等全局触发方式时按需使用；仅监听完成这些功能所需的全局事件，普通全局快捷键不需要这项授权。</li>
      ...
      <p><strong>键盘音效与输入统计：</strong>两项功能都默认关闭；开启键盘音效后只处理按键事件以播放声音，开启输入统计后只保存字符数、物理键码、时间和前台应用等聚合数据，不保存输入内容。你可以在设置中分别关闭并清除已保存的统计。</p>

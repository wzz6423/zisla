...
export const ko: SiteContent = {
  meta: {
    ...
    description:
      'zisla는 macOS를 위한 네이티브 다이내믹 워크스페이스입니다. Zed Agent 같은 AI 작업과 미디어, 파일, 일정을 한곳에서 확인하고 키보드 사운드, 입력 통계, 스크린샷 주석, 빠른 동작을 사용할 수 있습니다.',
    ogTitle: 'zisla · 지금 일어나는 일을 보이는 곳에',
    ogDescription:
      'Zed Agent 같은 AI 작업과 미디어 재생부터 키보드 사운드, 입력 통계, 빠른 동작, 스크린샷 주석, 데스크탑 도구까지. 필요할 때만 나타나는 네이티브 macOS 워크스페이스.',
  },
  ...
  hero: {
    ...
    lede: 'AI 작업과 미디어, 파일, 일정을 화면 위쪽에 모아 둡니다. 무언가를 복사하면 별도의 빠른 동작 표시줄이 화면 위에서 내용을 미리 보여 주고 다음에 할 일을 제안합니다. 필요할 때 나타나고, 끝나면 물러납니다.',
    downloadCta: '다운로드',
  ...
  showcase: {
    ...
    modules: {
      ...
      clipboard: {
        name: '클립보드',
        caption:
          '다이내믹 아일랜드 안에서 클립보드 기록을 보고 이미지, URL, 경로, 파일 종류로 걸러 보고, 각 항목을 복사한 시간도 확인할 수 있습니다. 항목은 빠른 메모로 보내거나 자주 쓰는 항목으로 지정하거나 삭제할 수 있습니다.',
      ...
      download: {
        name: '다운로더',
        caption:
          '링크를 붙여넣거나, 기능을 켠 뒤 클립보드에서 링크를 인식하게 할 수 있습니다. 이용 가능한 비디오 또는 오디오 형식을 선택하고, Douyin 다운로드에 필요한 경우 설치된 브라우저의 Cookies를 사용할 수 있습니다. 기본 또는 원하는 폴더로 저장하며 출처 아이콘, 실시간 진행률, 완료 상태를 함께 보여 줍니다.',
        points: ['사용 가능한 비디오 또는 오디오 형식 선택', 'Douyin용 브라우저 Cookies', '출처 아이콘과 실시간 진행률'],
      },
      agenda: {
        name: '일정과 날씨',
        caption:
      ...
      quickNotes: {
        name: '빠른 메모',
        caption:
          '시스템 "메모"를 데이터 원본으로 사용해 메모 보기, 편집, 새로 만들기, 삭제와 Markdown 실시간 미리보기를 지원하며, Command+1~9로 처음 아홉 개 메모로 바로 이동할 수 있습니다. 초안은 "메모"에 자동으로 다시 저장됩니다.',
      ...
      toolbox: {
        name: '작은 도구',
        caption:
          '집중 타이머, 화면 켜 두기, 화면 청소, 키보드 청소(청소 중에는 F1~F12를 포함한 키 입력을 막습니다), 프롬프터, 거울, 휴지통을 한 페이지에 모았습니다.',
  ...
  download: {
    eyebrow: '언제든 사용',
    title: 'zisla 다운로드',
    copy: 'Apple 실리콘 Mac용입니다. 버전, 다른 아키텍처, 체크섬은 릴리스 페이지에 있습니다. 설치한 뒤에는 선택한 업데이트 채널에서 새 버전을 확인할 수 있으며, Sparkle이 서명을 먼저 검증한 다음 설정에 따라 수동 또는 자동으로 다운로드·설치하고 다시 시작합니다. 업데이트 후에도 이미 허용한 권한은 유지되며, 업데이트 설정에는 GitHub Issues로 피드백을 보내는 바로가기도 있습니다. 필요 없는 릴리스는 건너뛰면 다시 알리지 않습니다.',
    primaryCta: '다운로드',
  ...
  faq: {
    ...
    items: {
      ...
      copyAssistant: {
        question: '빠른 동작이 복사한 내용을 자동으로 열거나 업로드하나요?',
        answer:
      ...
      <ul>
        ...
        <li><strong>알림:</strong> 포모도로 타이머를 켤 때 요청하며, 타이머가 끝날 때 로컬 알림을 표시하고 알림음을 한 번 재생하는 데만 사용합니다.</li>
      </ul>

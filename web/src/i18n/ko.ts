...
export const ko: SiteContent = {
  ...
  download: {
    eyebrow: '언제든 사용',
    title: 'zisla 다운로드',
    copy: 'Apple 실리콘 Mac용입니다. 버전, 다른 아키텍처, 체크섬은 릴리스 페이지에 있습니다. 설치한 뒤에는 선택한 업데이트 채널에서 새 버전을 확인할 수 있으며, Sparkle이 서명을 먼저 검증한 다음 설정에 따라 수동 또는 자동으로 다운로드·설치하고 다시 시작합니다. 업데이트 후에도 이미 허용한 권한은 유지되며, 필요 없는 릴리스는 건너뛰면 다시 알리지 않습니다.',
    primaryCta: '다운로드',
